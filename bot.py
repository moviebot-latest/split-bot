...
# Global dedup — defined at top so all handlers can use it
...
async def _dedup(mid: int) -> bool:
    """Atomic check — True means duplicate, skip it."""
...
# ══════════════════════════════════════════════════════════════
#  PROGRESS ENGINE v6
#  0.2s throttle · EMA speed · count-up % · flood-safe
# ══════════════════════════════════════════════════════════════
THROTTLE  = 0.2
EMA_ALPHA = 0.35
BAR_WIDTH = 20
...
# ══════════════════════════════════════════════════════════════
#  UPLOAD ONE PART  — FloodWait safe, no crash, no double
# ══════════════════════════════════════════════════════════════
    ...
    for attempt in range(5):
        ...
        try:
            ...
            await _safe_edit(status,
                ...
                f"  Telegram: wait `{wait}s`\n"
                f"  Resuming automatically…"
            )
            await asyncio.sleep(wait)   # wait & retry — NO crash
...
async def start(client, message):
    ...
    await message.reply(
        ...
        "🔄 FloodWait safe · No crash · No double upload"
    )
...
# ══════════════════════════════════════════════════════════════
#  RECEIVE VIDEO — all formats, forwarded, no double trigger
# ══════════════════════════════════════════════════════════════
